...
public class BarGraph extends View {
    ...
    public void onDraw(Canvas ca) {
        ...
        if (fullImage == null || shouldUpdate) {
            ...
            float padding = 7;
            int selectPadding = 4;
            float bottomPadding = 80;

            float usableHeight;
            if (showBarText) {
                this.p.setTextSize(60);
                this.p.getTextBounds(unit, 0, 0, r3);
                usableHeight = getHeight() - bottomPadding - Math.abs(r3.top - r3.bottom) - 46;
            } else {
            ...
            for (Bar p : points) {
                ...
                canvas.drawText(p.getName(), (int) (((r.left + r.right) / 2) - (this.p.measureText(p.getName()) / 2)), getHeight(), this.p);
                if (showBarText) {
                    this.p.setTextSize(60);
                    this.p.setColor(Color.WHITE);
                    this.p.getTextBounds(unit + p.getValue(), 0, 1, r2);
                    if (isAppended())
                        canvas.drawText(p.getValue() + unit, (int) (((r.left + r.right) / 2) - (this.p.measureText(unit + p.getValue()) / 2)), r.top, this.p);
                    else
                        canvas.drawText(unit + p.getValue(), (int) (((r.left + r.right) / 2) - (this.p.measureText(unit + p.getValue()) / 2)), r.top, this.p);
                }
